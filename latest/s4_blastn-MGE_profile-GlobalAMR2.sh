...
# 2. make_MGE_profile2.py [to count the reads of each MGE in the blast hits with >=90% of pident (% of identical positions) to >=25 bp of length, and to look up the gene information from the MGEDB catalog, and add in the read count data.]
#------------------------------------------------------------------------------
